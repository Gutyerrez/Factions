package net.hyren.factions.misc.player.list
...
object TabListManager {
    ...
    internal fun fetchByUserId(userId: EntityID<UUID>) = CACHE.getIfPresent(userId)

    internal fun fetchByUserId(userId: UUID) = CACHE.getIfPresent(
        EntityID(
            userId,
            FactionsUsersTable
        )
...
fun FactionUser.updatePlayerList() {
    ...
    if (hasFaction()) {
        ...
        CoreProvider.Cache.Redis.USERS_STATUS.provide().fetchUsersByServer(
            ...
            playerList.update(
                index, it?.getHighestGroup()?.getColoredPrefix() + it?.name
            )
...
fun FactionUser.uninitializedPlayerList() = TabListManager.delete(id)